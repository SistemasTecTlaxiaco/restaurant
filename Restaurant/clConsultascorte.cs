    ...
    public class clConsultascorte
    {
        public static string SiguienteCorte()
        {
            try
            {
                ...
                string consulta = "SELECT MAX(idCorte) FROM restaurante.Corte";
        ...
        public static clCorte ObtenerCorte(int corte)
        {
            ...
            {
                clConexion conexion = new clConexion();
                string consulta = "select * from restaurante.corte where idCorte = " + corte;
                MySqlCommand enviarSQL = new MySqlCommand(consulta, conexion.ObtenerConexion());
                enviarSQL.ExecuteNonQuery();
                //enviarSQL.ExecuteNonQuery();
                //obtener resultado
        ...
        {
            // SELECT * FROM restaurante.corte where Fecha = '2023-5-24'
            clConexion conexion = new clConexion();
            string consulta = "SELECT * FROM restaurante.corte where Fecha = '" + fecha + "'";
            MySqlCommand enviarSQL = new MySqlCommand(consulta, conexion.ObtenerConexion());
            enviarSQL.ExecuteNonQuery();
            MySqlDataReader lector = enviarSQL.ExecuteReader();
            lector.Read();
        ...
        {
            clConexion conexion = new clConexion();
            string consulta = "SELECT * FROM restaurante.corte where Turno = '" + turno + "'";
            MySqlCommand enviarSQL = new MySqlCommand(consulta, conexion.ObtenerConexion());
            enviarSQL.ExecuteNonQuery();
            MySqlDataReader lector = enviarSQL.ExecuteReader();
            lector.Read();
        ...
        {
            clConexion conexion = new clConexion();
            string consulta = "SELECT * FROM restaurante.corte where Cajero = '" + nombre + "'";
            MySqlCommand enviarSQL = new MySqlCommand(consulta, conexion.ObtenerConexion());
            enviarSQL.ExecuteNonQuery();
            MySqlDataReader lector = enviarSQL.ExecuteReader();
            lector.Read();
